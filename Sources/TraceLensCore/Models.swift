...
public enum ObservationRuleOrigin: String, Codable, Sendable {
  case configured, session, nextRequest
}
...
public struct SessionLimits: Sendable, Codable, Equatable {
  ...
  public init(
    maxTransactions: Int = 1_000, maxBodyBytes: Int = 5 * 1_024 * 1_024,
...
public struct BodyReference: Sendable, Codable, Equatable {
  ...
  public init(
    storage: Storage = .none, data: Data? = nil, fileName: String? = nil, originalSize: Int? = nil
  ) {
...
public struct ParsedEndpoint: Sendable, Codable, Equatable {
  ...
  public init(
    host: String, technicalService: String?, displayService: String?, endpoint: String,
    fullURL: String
  ) {
    self.host = host
...
public struct NetworkRequest: Sendable, Codable, Equatable {
  ...
  public init(
    url: URL, method: HTTPMethod, parsed: ParsedEndpoint, startedAt: Date = .now,
    headers: [String: String] = [:], body: BodyReference = .none, estimatedSize: Int? = nil
  ) {
    self.url = url
    self.method = method
    self.parsed = parsed
...
public struct NetworkResponse: Sendable, Codable, Equatable {
  ...
  public init(
    statusCode: Int, headers: [String: String] = [:], body: BodyReference = .none,
    mimeType: String? = nil, expectedContentLength: Int64? = nil, capturedSize: Int = 0,
    receivedAt: Date = .now
  ) {
...
public struct NetworkMetrics: Sendable, Codable, Equatable {
  ...
  public init(
    total: TimeInterval? = nil, dns: TimeInterval? = nil, tcp: TimeInterval? = nil,
    tls: TimeInterval? = nil, upload: TimeInterval? = nil, firstByte: TimeInterval? = nil,
    download: TimeInterval? = nil, protocolName: String? = nil, reusedConnection: Bool? = nil,
    redirectCount: Int = 0
  ) {
    self.total = total
    self.dns = dns
...
public struct NetworkTransaction: Identifiable, Sendable, Codable, Equatable {
  ...
  public init(
    id: UUID = UUID(), startedAt: Date = .now, request: NetworkRequest,
    response: NetworkResponse? = nil, metrics: NetworkMetrics? = nil, error: NetworkError? = nil,
    state: TransactionState = .running, captureLevel: CaptureLevel
  ) {
    self.id = id
    self.startedAt = startedAt
    self.finishedAt = nil
