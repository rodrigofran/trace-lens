...
public actor SessionStore {
  ...
  public func snapshot() -> SessionSnapshot {
    .init(
      session: session, transactions: order.reversed().compactMap { transactions[$0] },
      configuredRules: configuredRules, sessionRules: sessionRules, nextRules: nextRules,
  ...
  public func updates() -> AsyncStream<SessionSnapshot> {
    ...
    return AsyncStream { continuation in
      ...
      continuation.onTermination = { @Sendable _ in Task { await self.removeContinuation(token) } }
    }
  }
  ...
  public func resolve(url: URL, method: HTTPMethod, defaultCapture: CaptureLevel) -> (
    ...
    let matching = engine.resolve(
      url: url, method: method, configured: configuredRules, session: sessionRules, next: nextRules,
  ...
  public func begin(_ transaction: NetworkTransaction) -> Bool {
    guard transactions.count < limits.maxTransactions else { return false }
    transactions[transaction.id] = transaction
    order.append(transaction.id)
    if !transaction.request.parsed.host.isEmpty { hosts.insert(transaction.request.parsed.host) }
    publish()
    return true
  }
  ...
  public func update(_ id: UUID, _ change: @Sendable (inout NetworkTransaction) -> Void) {
    guard var value = transactions[id] else { return }
    change(&value)
