...
public actor CaptureRuntime {
  ...
  public func begin(id: UUID, request: URLRequest) async -> Bool {
    guard enabled, configuration.captureNetworkTraffic, let url = request.url, let store else {
      return false
    }
    ...
    let transaction = NetworkTransaction(
      id: id,
      request: .init(
        url: url, method: method, parsed: parsed, headers: headers, body: savedBody,
        estimatedSize: request.httpBody?.count), captureLevel: capture.0)
  ...
  public func response(
    ...
    guard let store, let transaction = await store.transaction(transactionID) else { return }
    ...
    await store.update(transaction.id) { value in
      value.response = .init(
        statusCode: http?.statusCode ?? 0, headers: headers, body: body,
  ...
  public func failed(_ error: Error, transactionID: UUID, cancelled: Bool = false) async {
    guard let store, let transaction = await store.transaction(transactionID),
...
public final class TraceLensURLProtocol: URLProtocol, URLSessionDataDelegate, @unchecked Sendable {
  ...
  public override func startLoading() {
    ...
    Task { await CaptureRuntime.shared.begin(id: id, request: request) }
  ...
  public func urlSession(
    ...
    if let error {
      ...
      Task { await CaptureRuntime.shared.failed(error, transactionID: id) }
    } else if let response = task.response {
      let captured = self.data
      client?.urlProtocolDidFinishLoading(self)
      Task { await CaptureRuntime.shared.response(response, data: captured, transactionID: id) }
    }
  }
}
...
public enum URLSessionInstrumentation {
  public static func instrument(_ configuration: URLSessionConfiguration) -> URLSessionConfiguration
  {
    ...
    guard copy.identifier == nil else { return copy }
