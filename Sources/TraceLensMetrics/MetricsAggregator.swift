...
public struct MetricsAggregator: Sendable {
  ...
  public func aggregate(_ transactions: [NetworkTransaction]) -> MetricsSnapshot {
    ...
    let errors = transactions.filter { $0.error != nil || ($0.response?.statusCode ?? 0) >= 400 }
      .count
    ...
    let services = grouped.map { key, values in
      ServiceMetric(
        service: key, requestCount: values.count,
        averageDuration: values.compactMap(\.duration).isEmpty
          ? nil
          : values.compactMap(\.duration).reduce(0, +) / Double(values.compactMap(\.duration).count)
      )
    ...
    ).map { TimeMetric(date: $0.key, count: $0.value.count) }.sorted { $0.date < $1.date }
    return .init(
      totalRequests: total,
      ...
      fullCaptureRatio: total == 0 ? 0 : Double(full) / Double(total), requestsOverTime: time,
      services: services)
  }
}
