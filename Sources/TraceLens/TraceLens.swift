...
private actor TraceLensCoordinator {
  ...
  func start(_ configuration: TraceLensConfiguration) async {
    if let bodies { await bodies.clear() }
    ...
    do {
      ...
      await MainActor.run {
        TraceLens.presentation = .init(
          store: store, configuration: configuration, visible: TraceLens.presentation.visible)
      }
    } catch {
      self.store = store
      self.bodies = nil
  ...
  func stop() async {
    ...
    store = nil
    bodies = nil
    await MainActor.run { TraceLens.presentation = .init() }
  }
  func clear() async {
    if let store { await store.clear() }
    if let bodies { await bodies.clear() }
  }
  func update(_ configuration: TraceLensConfiguration) async {
    ...
    await MainActor.run {
      TraceLens.presentation = .init(
        store: currentStore, configuration: configuration, visible: TraceLens.presentation.visible)
    }
  }
  func export() async throws -> URL {
    guard let store else { throw TraceLensError.notStarted }
    let snapshot = await store.snapshot()
    let root = FileManager.default.temporaryDirectory.appending(
      path: "TraceLensSession-\(snapshot.session.id.uuidString).tracelens",
      directoryHint: .isDirectory)
    try? FileManager.default.removeItem(at: root)
    try FileManager.default.createDirectory(
      at: root.appending(path: "requests", directoryHint: .isDirectory),
      withIntermediateDirectories: true)
    try FileManager.default.createDirectory(
      at: root.appending(path: "responses", directoryHint: .isDirectory),
      withIntermediateDirectories: true)
    ...
    if configuration.sensitiveDataPolicy == .redacted {
      for index in exported.indices {
        exported[index].request.headers = SensitiveData.headers(
          exported[index].request.headers, policy: .redacted)
    ...
    try encoder.encode(exported).write(to: root.appending(path: "transactions.json"))
    let metrics = MetricsAggregator().aggregate(snapshot.transactions)
    let metricData = try JSONSerialization.data(
      withJSONObject: [
        "totalRequests": metrics.totalRequests, "errorRate": metrics.errorRate,
        "fullCaptureRatio": metrics.fullCaptureRatio,
      ], options: [.prettyPrinted, .sortedKeys])
    try metricData.write(to: root.appending(path: "metrics.json"))
...
public enum TraceLensError: Error, LocalizedError {
  case notStarted
  public var errorDescription: String? { "O TraceLens ainda não foi iniciado." }
}
...
@MainActor private struct Presentation {
...
public enum TraceLens {
  ...
  public static func start(configuration: TraceLensConfiguration = .init()) {
    Task { await coordinator.start(configuration) }
  }
  ...
  public static func updateConfiguration(_ configuration: TraceLensConfiguration) {
    Task { await coordinator.update(configuration) }
  }
...
public enum CurlExporter {
  public static func command(for transaction: NetworkTransaction, policy: SensitiveDataPolicy)
    -> String?
  {
    guard transaction.captureLevel == .full else { return nil }
    var parts = [
      "curl", "-X", transaction.request.method.rawValue,
    ...
    if policy == .visible, case .inline = transaction.request.body.storage,
      let data = transaction.request.body.data, let body = String(data: data, encoding: .utf8)
    {
