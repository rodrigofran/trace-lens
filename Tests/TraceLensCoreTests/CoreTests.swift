...
final class CoreTests: XCTestCase {
  func testMatcherAndSpecificity() throws {
    ...
    XCTAssertTrue(
      RequestMatcher(host: "api.example.test", pathPrefix: "/payments", methods: [.post]).matches(
        url, method: .post))
    XCTAssertFalse(
  ...
  func testRulePriorityAndParser() throws {
    ...
    XCTAssertEqual(
      engine.captureLevel(
        url: url, method: .post, configured: [configured], session: [], next: [next],
        defaultCapture: .metadata), .none)
    let parsed = EndpointParser(
      strategy: .serviceAfterPathPrefix("/api/v2/demo"), aliases: ["payment-service": "Payments"]
    ).parse(url)
    ...
    let versionedURL = try XCTUnwrap(
      URL(string: "https://api.example.test/v2/sicredi/payments/orders"))
    let versioned = EndpointParser(
      strategy: .serviceAtPathIndex(2), aliases: ["payments": "Pagamentos"]
    ).parse(versionedURL)
    XCTAssertEqual(versioned.displayService, "Pagamentos")
    XCTAssertEqual(versioned.endpoint, "/orders")
  }
  func testSensitiveData() {
    XCTAssertEqual(
      SensitiveData.value("secret", key: "Authorization", policy: .redacted), "••••••••")
    XCTAssertEqual(SensitiveData.value("ok", key: "Accept", policy: .redacted), "ok")
  }
}
